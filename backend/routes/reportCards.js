...
router.post("/", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    const processedSubjects = (subjects || []).map((s) => {
      ...
      const hasEntry = ca > 0 || exam > 0;

      // Total = average of entered components (both are /100)
      let total = 0;
      if (hasEntry) {
        const count = (ca > 0 ? 1 : 0) + (exam > 0 ? 1 : 0);
        total = count > 0 ? Math.round((ca + exam) / count) : 0;
      }

      // Zambian grade system — blank if no marks entered
      let grade_letter = "";
      if (total >= 80)      grade_letter = "D";   // Distinction
      else if (total >= 65) grade_letter = "M";   // Merit
      else if (total >= 50) grade_letter = "CR";  // Credit
      else if (total >= 40) grade_letter = "S";   // Satisfactory
      else if (total >= 30) grade_letter = "AP";  // Approaching
      else if (total > 0)   grade_letter = "U";   // Unsatisfactory

      return {
        name: s.name,
        continuous_assessment: ca,
        end_of_term_exam: exam,
        total: hasEntry ? total : null,  // null = not entered
        grade_letter: hasEntry ? grade_letter : "",
