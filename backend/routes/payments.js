...
router.post("/", protect, async (req, res) => {
  try {
    ...
    if (paymentType === "school_fee_termly") {
      ...
      const evDoc = await Event.findById(req.body.eventId);
      if (evDoc) {
        const d = new Date(evDoc.eventDate);
        d.setDate(d.getDate() + 7);
        expiresAt = d;
        calculatedAmount = evDoc.paymentAmount || Number(amount) || 0;
      } else {
...
router.get("/fees", protect, async (req, res) => {
  try {
    ...
    if (!fees) fees = await FeeSettings.create({});
    res.json({ fees });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put("/fees", protect, authorize("admin"), async (req, res) => {
  try {
    ...
    let fees = await FeeSettings.findOne();
    if (!fees) fees = new FeeSettings({});
    if (schoolFeeMonthly !== undefined) fees.schoolFeeMonthly = +schoolFeeMonthly;
    if (schoolFeeTermly  !== undefined) fees.schoolFeeTermly  = +schoolFeeTermly;
    if (testFeeLower     !== undefined) fees.testFeeLower     = +testFeeLower;
    if (testFeeUpper     !== undefined) fees.testFeeUpper     = +testFeeUpper;
    fees.updatedBy = req.user._id;
    await fees.save();
    req.app.get("io").emit("fees_updated", { fees });
    res.json({ fees });
  } catch (err) { res.status(500).json({ error: err.message }); }
});


// ── POST /api/payments/:id/installment - Add installment to partial payment
router.post("/:id/installment", protect, async (req, res) => {
  try {
    const { amount, mobileMoneyRef, proofImageData, proofImageMime } = req.body;
    const payment = await Payment.findById(req.params.id);
    ...
    payment.paidAmount = (payment.paidAmount || 0) + addAmount;
    payment.remainingBalance = Math.max(0, (payment.fullAmount || payment.amount) - payment.paidAmount);
    payment.installments = payment.installments || [];
    payment.installments.push({ amount: addAmount, date: new Date(), ref: mobileMoneyRef || "" });
    if (proofImageData) { payment.proofImageData = proofImageData; payment.proofImageMime = proofImageMime; }
    ...
    if (payment.remainingBalance <= 0) {
      io.to(`user:${payment.parent}`).emit("payment_approved", {
        payment, message: "✅ Payment complete! Access activated."
      });
    } else {
      io.to(`user:${payment.parent}`).emit("balance_updated", {
        paymentId: payment._id,
        paidAmount: payment.paidAmount,
        remainingBalance: payment.remainingBalance,
        message: `💳 ZMW ${addAmount} received. Remaining: ZMW ${payment.remainingBalance.toFixed(2)}`,
      });
    }
    res.json({ payment });
  } catch (err) { res.status(500).json({ error: err.message }); }
...
// ── GET /api/payments/admin/all - fixed route
