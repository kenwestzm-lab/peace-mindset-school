...
// ✅ FIX: No silent fallback — if Cloudinary fails, surface the real error
const processImage = async (imageData, folder, publicId) => {
  if (!imageData) throw new Error("No image provided");
  // Already a URL (e.g. existing Cloudinary link) — return as-is
  ...
  // Upload to Cloudinary — throws with real error message if it fails
