...
router.post("/upload", protect, async (req, res) => {
  try {
    ...
    // Audio/voice fallback: return base64 so voice notes never fail with 500
    if ((mimeType || '').startsWith('audio/') || (folder || '').includes('voice')) {
      console.warn('Cloudinary audio fallback:', err.message);
      return res.json({ url: mediaData, publicId: null, fallback: true });
    }
    // Audio/voice fallback: return base64 so voice notes never fail with 500
    if ((mimeType || '').startsWith('audio/') || (folder || '').includes('voice')) {
      console.warn('Cloudinary audio fallback:', err.message);
