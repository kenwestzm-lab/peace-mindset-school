...
io.on("connection", (socket) => {
  ...
  socket.on("send_group_message", async (data) => {
    try {
      ...
      // Check group membership for parents
      const group = await Group.findById(data.groupId);
      if (!group) return;
      const isMember = data.senderRole === 'admin' ||
        group.members.some(m => m.toString() === data.senderId?.toString());
      if (!isMember) {
        socket.emit('error', { message: 'Not a group member' });
        return;
      }
      // Check send permission
      if (data.senderRole !== 'admin' && group.permissions?.membersCanSend === false) {
        socket.emit('error', { message: 'Members cannot send messages in this group' });
        return;
      }
