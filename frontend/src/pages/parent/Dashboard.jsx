...
import GoldSlideshow from '../../components/GoldSlideshow';
...
export default function ParentDashboard() {
  ...
  // Listen for child photo updates in real time
  useEffect(() => {
    const s = getSocket();
    if (s) {
      s.on('child_updated', ({ childId, profilePic }) => {
        setData(d => ({
          ...d,
          children: d.children.map(c => c._id === childId ? { ...c, profilePic } : c)
        }));
      });
    }
    return () => getSocket()?.off('child_updated');
  }, []);
  ...
  return (
    <div style={{ padding:'20px 16px 80px', maxWidth:700, margin:'0 auto' }}>
      ...
      {/* Chat with Admin */}
      <Link to="/parent/chat" style={{textDecoration:'none',display:'block',marginBottom:18}}>
        <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,padding:'16px 18px',display:'flex',alignItems:'center',gap:14,cursor:'pointer'}}>
          <div style={{width:48,height:48,borderRadius:'50%',background:'linear-gradient(135deg,#1565C0,#0D47A1)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>🏫</div>
          <div style={{flex:1}}>
            <div style={{fontSize:15,fontWeight:700,color:'var(--text)'}}>💬 Chat with School</div>
            <div style={{fontSize:12,color:'var(--text-muted)',marginTop:2}}>Send messages, voice notes and photos to admin</div>
          </div>
