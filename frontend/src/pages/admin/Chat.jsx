  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 180 }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {(dur && isFinite(dur) && !isNaN(dur)) ? `${Math.floor(dur / 60)}:${String(Math.floor(dur % 60)).padStart(2, "0")}` : "0:00"}
        </div>
      </div>
...
/* ─── AddMembersPanel ─────────────────────────────────────────────── */
function AddMembersPanel({ groupId, currentMembers, onAdded }) {
  ...
  const search = async (q) => {
    ...
    try {
      const r = await api.get(`/admin/parents?search=${encodeURIComponent(q)}`);
      const all = r.data.parents || [];
      const currentIds = currentMembers.map(m => m._id);
      setResults(all.filter(p => !currentIds.includes(p._id)).slice(0, 5));
    } catch {}
  };

  const addMember = async (parent) => {
    setAdding(parent._id);
    try {
      await api.post(`/groups/${groupId}/members`, { members: [parent._id] });
      onAdded(parent);
      setResults(r => r.filter(p => p._id !== parent._id));
      toast.success(`${parent.name} added!`);
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 12, color: '#00A884', fontWeight: 600, marginBottom: 8, letterSpacing: .4 }}>ADD MEMBERS</div>
      <input
        value={query}
        onChange={e => search(e.target.value)}
        placeholder="🔍 Search parent by name or email..."
        style={{ width: '100%', background: '#2A3942', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '10px 14px', color: '#E9EDEF', fontSize: 14, outline: 'none', boxSizing: 'border-box', marginBottom: 8 }}
      />
      {results.map(p => (
        <div key={p._id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#2A3942,#3B4A54)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#fff', overflow: 'hidden', flexShrink: 0 }}>
            {p.profilePic ? <img src={p.profilePic} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : p.name?.[0]?.toUpperCase()}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 600 }}>{p.name}</div>
            <div style={{ fontSize: 12, color: '#8696A0' }}>{p.email}</div>
          </div>
          <button
            onClick={() => addMember(p)}
            disabled={adding === p._id}
            style={{ background: '#00A884', border: 'none', color: '#fff', borderRadius: 8, padding: '6px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}
          >
            {adding === p._id ? '...' : '+ Add'}
          </button>
        </div>
    ...
    <div style={root}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {tab === 'info' ? (
          ...
          <>
            {/* Add Members Search */}
            <AddMembersPanel groupId={group._id} currentMembers={members} onAdded={(newM) => setMembers(m => [...m, newM])} />
            {members.map(m => (
...
export default function AdminChat() {
  ...
  const loadMessages = useCallback(async pid => {
    if (!pid) return;
    try { const r = await api.get(`/chat/${pid}`); setMessages(r.data.messages || []); setUnread(u => ({ ...u, [pid]: 0 })); } catch (e) { console.error('msgs', e); }
  });
  ...
  useEffect(() => {
    ...
    socket.on('group_photo_updated', onGrpPhoto);
    socket.on('group_updated', ({groupId, photo, name}) => {
      setGroups(p => p.map(g => g._id === groupId ? {...g,...(photo&&{photo}),...(name&&{name})} : g));
      setSelGroup(sg => sg && sg._id === groupId ? {...sg,...(photo&&{photo}),...(name&&{name})} : sg);
    });
    return () => { socket.off('new_message', onMsg); socket.off('user_typing', onTyping); socket.off('message_deleted', onDel); socket.off('message_reaction', onReact); socket.off('new_group_message', onGrpMsg); socket.off('new_story', loadStories); socket.off('stories_expired', loadStories); socket.off('group_photo_updated', onGrpPhoto); socket.off('group_updated'); };
  ...
  const startRec = async () => {
    try {
      ...
      mr.onstop = async () => {
        ...
        try {
          let voiceUrl = null;
          try{ const result = await uploadMedia(file); voiceUrl = result.url; }
          catch(e){ const b64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=ev=>res(ev.target.result);r.onerror=rej;r.readAsDataURL(file);}); voiceUrl=b64; }
          const p = { senderId: user?._id, senderRole: 'admin', content: '', messageType: 'voice', mediaData: voiceUrl, mediaMimeType: mimeType };
          if (selGroup && panel === 'group') socket.emit('send_group_message', { ...p, groupId: selGroup._id });
          else if (selParent) socket.emit('send_message', { ...p, parentId: selParent._id });
          // Optimistic update
          const vOpt = { _id: 'tmp_'+Date.now(), sender: { _id: user?._id, name: user?.name, profilePic: user?.profilePic }, senderRole: 'admin', ...p, createdAt: new Date().toISOString(), isRead: true };
          if (selGroup && panel === 'group') setGroupMsgs(prev => [...prev, vOpt]);
          else setMessages(prev => [...prev, vOpt]);
  ...
  /* ─── Recording Bar ─── */
  const RecordingBar = () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: '#1F2C34', flexShrink: 0 }}>
      <button onClick={cancelRec} style={{ ...iconBtnStyle, color: '#EF4444', background: 'rgba(239,68,68,0.15)', flexShrink: 0 }}>🗑</button>
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: '#2A3942', borderRadius: 24, padding: '8px 12px', minWidth: 0, overflow: 'hidden' }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#EF4444', flexShrink: 0, animation: 'blink2 1s infinite' }} />
        <span style={{ fontSize: 13, color: '#EF4444', fontWeight: 700, minWidth: 34, flexShrink: 0 }}>
      ...
      <button
        onClick={stopRec}
        style={{ ...sendBtnStyle, background: '#00A884', flexShrink: 0, boxShadow: '0 2px 8px rgba(0,168,132,0.4)' }}
      ...
      <style>{`@keyframes blink2{0%,100%{opacity:1}50%{opacity:0.2}}`}</style>
    </div>
  );
