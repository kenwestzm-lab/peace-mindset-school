...
const fmtDay=(d)=>{if(!d)return'';const diff=Math.floor((Date.now()-new Date(d))/86400000);if(diff===0)return'Today';if(diff===1)return'Yesterday';return new Date(d).toLocaleDateString('en-GB',{day:'2-digit',month:'short'});};
const timeLeft=(exp)=>{const ms=new Date(exp)-Date.now();if(ms<=0)return'Expired';const h=Math.floor(ms/3600000),m=Math.floor((ms%3600000)/60000);return h>0?`${h}h ${m}m left`:`${m}m left`;};
...
function VoiceMsg({src}){
  const [playing,setPlaying]=useState(false);const [prog,setProg]=useState(0);const [dur,setDur]=useState(0);const ref=useRef(null);
  const toggle=()=>{if(!ref.current)return;if(playing){ref.current.pause();setPlaying(false);}else{ref.current.play();setPlaying(true);}};
  return(<div style={{display:'flex',alignItems:'center',gap:8,minWidth:180}}>
    ...
    <div style={{flex:1}}><div style={{height:3,background:'rgba(255,255,255,0.2)',borderRadius:2,overflow:'hidden'}}><div style={{height:'100%',width:`${prog}%`,background:'#25D366',transition:'width .1s'}}/></div><div style={{fontSize:10,color:'rgba(255,255,255,0.5)',marginTop:3}}>{dur?`${Math.floor(dur/60)}:${String(Math.floor(dur%60)).padStart(2,'0')}`:'0:00'}</div></div>
    <span style={{fontSize:18}}>🎤</span>
  </div>);}

function Bubble({msg,isMe,showName,onLongPress}){
  const [big,setBig]=useState(false);const pt=useRef(null);const moved=useRef(false);const deleted=msg.deletedForEveryone;
  ...
  return(<>
    <div style={{display:'flex',justifyContent:isMe?'flex-end':'flex-start',marginBottom:2,paddingLeft:isMe?48:8,paddingRight:isMe?8:48}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE} onMouseDown={onTS} onMouseUp={onTE}>
      {!isMe&&<div style={{...av(28),marginRight:5,alignSelf:'flex-end'}}>{msg.sender?.profilePic?<img src={msg.sender.profilePic} style={avImg} alt=""/>:<span style={{fontSize:11}}>{msg.sender?.name?.[0]?.toUpperCase()||'P'}</span>}</div>}
...
function StoryViewer({groups,groupIdx:initGI,onClose,currentUserId}){
  ...
  useEffect(()=>{if(!story)return;api.put(`/stories/${story._id}/view`).catch(()=>{});setPct(0);const dur=story.mediaType==='video'?15000:5000;const step=100/(dur/50);timer.current=setInterval(()=>setPct(p=>{if(p>=100){advance();return 100;}return p+step;}),50);return()=>clearInterval(timer.current);},[gi,si]);
  ...
  return(<div style={{position:'fixed',inset:0,background:'#000',zIndex:9999,display:'flex',flexDirection:'column'}} onClick={tap}>
    <div style={{display:'flex',gap:3,padding:'12px 12px 0',flexShrink:0}}>{group.items.map((_,i)=>(<div key={i} style={{flex:1,height:3,background:'rgba(255,255,255,0.3)',borderRadius:2,overflow:'hidden'}}><div style={{height:'100%',background:'#fff',width:`${i<si?100:i===si?pct:0}%`}}/></div>))}</div>
...
function CreateGroupSheet({onClose,onCreated,allParents}){
  const [name,setName]=useState('');const [desc,setDesc]=useState('');const [creating,setCreating]=useState(false);
  const create=async()=>{if(!name.trim()){toast.error('Enter a group name');return;}setCreating(true);try{await api.post('/groups',{name:name.trim(),description:desc.trim()});toast.success('Group created!');onCreated();}catch(e){toast.error(e.response?.data?.error||'Failed');}finally{setCreating(false);}};
  return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:9000,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={onClose}>
    <div style={{background:'#1F2C34',borderRadius:'20px 20px 0 0',padding:'20px 16px 36px',width:'100%',maxWidth:500}} onClick={e=>e.stopPropagation()}>
      ...
      <p style={{fontSize:12,color:'#8696A0',marginBottom:14}}>All parents can see and join this group. You can add members after creating.</p>
...
function CreateStory({onClose,onPosted}){
  ...
  const post=async()=>{if(!file&&!text.trim()){toast.error('Add photo, video or text');return;}setPosting(true);try{let md=null,mm=null,mt='text';if(file){await new Promise((res,rej)=>{const r=new FileReader();r.onload=e=>{md=e.target.result;res();};r.onerror=rej;r.readAsDataURL(file);});mm=file.type;mt=file.type.startsWith('video/')?'video':'image';}await api.post('/stories',{mediaData:md,mediaMimeType:mm,mediaType:mt,text:text||null});toast.success('Status posted!');onPosted();}catch(e){toast.error(e.response?.data?.error||'Failed');}finally{setPosting(false);}};
  return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.65)',zIndex:9000,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={onClose}>
    <div style={{background:'#1F2C34',borderRadius:'20px 20px 0 0',padding:'20px 16px 36px',width:'100%',maxWidth:500}} onClick={e=>e.stopPropagation()}>
      ...
      {prev?<div style={{position:'relative',marginBottom:12,borderRadius:12,overflow:'hidden'}}>{file?.type.startsWith('video/')?<video src={prev} style={{width:'100%',maxHeight:200,objectFit:'cover'}} muted/>:<img src={prev} style={{width:'100%',maxHeight:200,objectFit:'cover'}} alt=""/>}<button onClick={()=>{setFile(null);setPrev(null);}} style={{position:'absolute',top:6,right:6,width:26,height:26,borderRadius:'50%',background:'rgba(0,0,0,0.6)',border:'none',color:'#fff',fontSize:14,cursor:'pointer'}}>✕</button></div>:<button onClick={()=>ref.current?.click()} style={{width:'100%',padding:'20px 0',background:'#2A3942',border:'2px dashed rgba(255,255,255,0.12)',borderRadius:12,color:'#8696A0',cursor:'pointer',fontSize:14,marginBottom:12}}>📷 Add Photo or Video</button>}
...
export default function AdminChat(){
  const {user}=useStore();
  const [tab,setTab]=useState('chats'); // chats | updates
  const [panel,setPanel]=useState('list'); // list | dm | group
  ...
  const [recording,setRecording]=useState(false);const [recObj,setRecObj]=useState(null);
  const [menu,setMenu]=useState(null);
  const [showNewGroup,setShowNewGroup]=useState(false);
  const [storyGroups,setStoryGroups]=useState([]);const [myStories,setMyStories]=useState([]);const [storyViewer,setStoryViewer]=useState(null);const [showCreate,setShowCreate]=useState(false);
  const bottomRef=useRef(null);const fileRef=useRef(null);const grpFileRef=useRef(null);const typingTimer=useRef(null);

  const loadParents=useCallback(async()=>{try{const r=await api.get('/admin/parents');setParents(r.data.parents||[]);}catch(e){console.error('parents',e);}});
  const loadMessages=useCallback(async(pid)=>{if(!pid)return;try{const r=await api.get(`/chat/messages/${pid}`);setMessages(r.data.messages||[]);setUnread(u=>({...u,[pid]:0}));}catch(e){console.error('msgs',e);}});
  const loadGroups=useCallback(async()=>{try{const r=await api.get('/groups');setGroups(r.data.groups||[]);}catch(e){console.error('groups',e);}});
  const loadGroupMsgs=useCallback(async(gid)=>{try{const r=await api.get(`/groups/${gid}/messages`);setGroupMsgs(r.data.messages||[]);}catch(e){console.error('grpmsgs',e);}});
  const loadStories=useCallback(async()=>{try{const r=await api.get('/stories');const all=r.data.stories||[];const map={};all.forEach(s=>{const k=s.author?._id;if(!map[k])map[k]={author:s.author,items:[]};map[k].items.push(s);});setMyStories(map[user?._id]?.items||[]);setStoryGroups(Object.values(map).filter(g=>g.author?._id!==user?._id));}catch(e){console.error('stories',e);}});
  ...
  useEffect(()=>{
    ...
    const onMsg=(msg)=>{const pid=msg.parentId;if(selParent?._id===pid){setMessages(p=>[...p,msg]);api.put(`/chat/${msg._id}/read`).catch(()=>{});}else setUnread(u=>({...u,[pid]:(u[pid]||0)+1}));};
    ...
    const onGrpMsg=(msg)=>{if(selGroup?._id===msg.group)setGroupMsgs(p=>[...p,msg]);setGroups(p=>p.map(g=>g._id===msg.group?{...g,lastMessage:msg.content||'📎',lastMessageTime:msg.createdAt}:g));};
  ...
  // CRITICAL FIX: state update is always first, socket is secondary
  const handleInput=(e)=>{
  ...
  const sendText=()=>{const txt=input.trim();if(!txt||!selParent)return;const socket=getSocket();if(socket)socket.emit('send_message',{senderId:user?._id,senderRole:'admin',parentId:selParent._id,content:txt,messageType:'text'});setInput('');};
  const sendGrpText=()=>{const txt=grpInput.trim();if(!txt||!selGroup)return;const socket=getSocket();if(socket)socket.emit('send_group_message',{groupId:selGroup._id,senderId:user?._id,senderRole:'admin',content:txt,messageType:'text'});setGrpInput('');};
  const sendMedia=async(file,isGrp=false)=>{if(!file)return;if(!file.type.startsWith('image/')&&!file.type.startsWith('video/')){toast.error('Images/videos only');return;}const reader=new FileReader();reader.onload=e=>{const socket=getSocket();if(!socket)return;const p={senderId:user?._id,senderRole:'admin',content:'',messageType:file.type.startsWith('image/')?'image':'video',mediaData:e.target.result,mediaMimeType:file.type};if(isGrp&&selGroup)socket.emit('send_group_message',{...p,groupId:selGroup._id});else if(selParent)socket.emit('send_message',{...p,parentId:selParent._id});};reader.readAsDataURL(file);};
  const startRec=async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});const mimeType=MediaRecorder.isTypeSupported('audio/webm;codecs=opus')?'audio/webm;codecs=opus':MediaRecorder.isTypeSupported('audio/webm')?'audio/webm':'audio/mp4';const mr=new MediaRecorder(stream,{mimeType});const ch=[];mr.ondataavailable=e=>{if(e.data.size>0)ch.push(e.data);};mr.onstop=()=>{const blob=new Blob(ch,{type:mimeType});const reader=new FileReader();reader.onload=e=>{const socket=getSocket();if(!socket||!selParent)return;socket.emit('send_message',{senderId:user?._id,senderRole:'admin',parentId:selParent._id,content:'',messageType:'voice',mediaData:e.target.result,mediaMimeType:mimeType});stream.getTracks().forEach(t=>t.stop());};reader.readAsDataURL(blob);};mr.start(100);setRecObj(mr);setRecording(true);}catch(err){if(err.name==='NotAllowedError'){toast.error('Microphone permission denied');}else{toast.error('Mic error: '+err.message);}}};
  const stopRec=()=>{recObj?.stop();setRecording(false);setRecObj(null);};
  const doDelete=async(msgId,forAll)=>{try{await api.delete(`/chat/${msgId}`,{data:{forEveryone:forAll}});if(forAll)setMessages(p=>p.map(m=>m._id===msgId?{...m,deletedForEveryone:true}:m));else setMessages(p=>p.filter(m=>m._id!==msgId));setMenu(null);}catch{toast.error('Delete failed');}};
  ...
  if(panel==='dm'&&selParent)return(
    <div style={root}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'6px 0 4px'}}>
        ...
        {groupMsgsByDay(messages).map((grp,gi)=>(
          <div key={gi}>
            ...
            {grp.msgs.map(m=>{const isMe=m.senderRole==='admin';return<Bubble key={m._id} msg={m} isMe={isMe} showName={false} onLongPress={msg=>setMenu({msg})}/>;})}</div>))}
        <div ref={bottomRef}/>
      </div>
      <div style={inputBarStyle}>
        ...
        {input.trim()?<button onClick={sendText} style={sendBtnStyle}>➤</button>
          :<button style={{...sendBtnStyle,background:recording?'#EF4444':'#00A884'}} onMouseDown={startRec} onMouseUp={stopRec} onTouchStart={e=>{e.preventDefault();startRec();}} onTouchEnd={stopRec}>{recording?'⏹':<svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="9" y="2" width="6" height="12" rx="3" fill="#fff"/><path d="M5 11C5 14.866 8.13401 18 12 18C15.866 18 19 14.866 19 11" stroke="#fff" strokeWidth="2" strokeLinecap="round"/><line x1="12" y1="18" x2="12" y2="22" stroke="#fff" strokeWidth="2" strokeLinecap="round"/><line x1="8" y1="22" x2="16" y2="22" stroke="#fff" strokeWidth="2" strokeLinecap="round"/></svg>}</button>}
      </div>
    </div>);
  ...
  if(panel==='group'&&selGroup)return(
    <div style={root}>
      <CtxMenu/>
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'#1F2C34',flexShrink:0,borderBottom:'0.5px solid rgba(255,255,255,0.06)'}}>
        <button onClick={()=>{setPanel('list');setSelGroup(null);setGroupMsgs([]);}} style={{background:'none',border:'none',color:'#8696A0',fontSize:22,cursor:'pointer',padding:'0 8px 0 0'}}>←</button>
        ...
        <div style={{flex:1,overflow:'hidden'}}>
          ...
          <div style={{fontSize:12,color:'#8696A0'}}>{selGroup.members?.length||0} members</div>
        </div>
      </div>
      <div style={{flex:1,overflowY:'auto',padding:'6px 0 4px'}}>
        ...
        {groupMsgsByDay(groupMsgs).map((grp,gi)=>(
          <div key={gi}>
            <div style={{textAlign:'center',margin:'8px 0'}}><span style={{background:'#1F2C34',padding:'4px 12px',borderRadius:8,fontSize:11,color:'#8696A0',border:'0.5px solid rgba(255,255,255,0.06)'}}>{grp.day}</span></div>
            {grp.msgs.map(m=>{const isMe=(m.sender?._id||m.sender)===user?._id;return<Bubble key={m._id} msg={m} isMe={isMe} showName={!isMe} onLongPress={msg=>setMenu({msg})}/>;})}</div>))}
        <div ref={bottomRef}/>
      </div>
      <div style={inputBarStyle}>
  ...
  return(
    <div style={root}>
      {showNewGroup&&<CreateGroupSheet onClose={()=>setShowNewGroup(false)} onCreated={()=>{setShowNewGroup(false);loadGroups();}} allParents={parents}/>}
      ...
      {tab==='updates'?(
        <div style={{flex:1,overflowY:'auto'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12,padding:'10px 16px',cursor:'pointer',borderBottom:'0.5px solid rgba(255,255,255,0.05)'}} onClick={myStories.length?()=>setStoryViewer(0):()=>setShowCreate(true)}>
            ...
            <div><div style={{fontSize:15,fontWeight:600}}>My status</div><div style={{fontSize:13,color:'#8696A0'}}>{myStories.length?`${myStories.length} update${myStories.length>1?'s':''}·Tap to view`:'Tap to add status update'}</div></div>
          </div>
        ...
        <>
          ...
          <div style={{flex:1,overflowY:'auto'}}>
            ...
            {groups.map(g=>(
              <div key={g._id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',cursor:'pointer',borderBottom:'0.5px solid rgba(255,255,255,0.04)'}} onClick={()=>{setSelGroup(g);loadGroupMsgs(g._id);setPanel('group');}}>
            ...
            {filtered.map(p=>{const u=unread[p._id]||0;const typing=parentTyping[p._id];
              return(<div key={p._id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',cursor:'pointer',borderBottom:'0.5px solid rgba(255,255,255,0.04)'}} onClick={()=>{setSelParent(p);setPanel('dm');}}>
