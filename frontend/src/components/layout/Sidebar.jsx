      ...
      <aside className={`sidebar${mobileOpen?' open':''}`} style={{
        ...
        <div style={{ padding:'18px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:11 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:8.5, letterSpacing:'0.14em', color:'rgba(212,168,67,0.8)', marginTop:2, textTransform:'uppercase', fontFamily:'var(--font-title)' }}>
                Private School
              </div>
            </div>
          </div>
        ...
        <div style={{ padding:'11px 12px 8px', flexShrink:0 }}>
          <div style={{
            ...
            <div style={{
              ...
              boxShadow:'0 0 10px rgba(155,24,38,0.4)',
            }}>{user?.name?.[0]?.toUpperCase()}</div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:12.5, fontWeight:600, color:'#FFFFFF', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                {user?.name}
              </div>
              <div style={{ fontSize:9.5, fontWeight:700, color:roleColor, textTransform:'uppercase', letterSpacing:'0.06em', marginTop:1 }}>
                {roleLabel}
              </div>
            </div>
          </div>
        ...
        <nav style={{ flex:1, padding:'2px 8px 8px', overflowY:'auto' }}>
          {navItems.map(item => (
            <NavLink key={item.key} to={item.path} end={item.exact} onClick={onClose}
              ...
              onMouseEnter={e=>{ if(!e.currentTarget.style.background.includes('rgba(155,24,38,0.35')) { e.currentTarget.style.background='rgba(255,255,255,0.05)'; e.currentTarget.style.color='rgba(255,255,255,0.85)'; } }}
              onMouseLeave={e=>{ if(!e.currentTarget.style.background.includes('rgba(155,24,38,0.35')) { e.currentTarget.style.background='transparent'; e.currentTarget.style.color='rgba(255,255,255,0.55)'; } }}
            >
        ...
        <div style={{ padding:'10px 8px 16px', borderTop:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
          <button
            ...
            onMouseEnter={e=>{ e.currentTarget.style.background='rgba(239,68,68,0.2)'; e.currentTarget.style.color='#FCA5A5'; e.currentTarget.style.borderColor='rgba(239,68,68,0.5)'; }}
            onMouseLeave={e=>{ e.currentTarget.style.background='rgba(239,68,68,0.1)'; e.currentTarget.style.color='#FC8181'; e.currentTarget.style.borderColor='rgba(239,68,68,0.25)'; }}
          >
